"""Small CPU PPO implementation with continuous motion and masked handovers."""
...
class HybridPolicy(nn.Module):
    def __init__(self, obs_dim: int, hidden: int = 64):
        ...
        self.network = nn.Linear(hidden, 5)
...
def evaluate(policy, radio, scenarios, env_config, out_prefix=None, pd=False,
             reward="fixed", terminate_success=True, traces=4):
    env = UAVBatch(radio, len(scenarios), scenarios, reward, terminate_success,
...
def summarize(results):
    ...
    summary = {"episodes": len(results), "success_rate": len(success)/len(results),
        "feasible_success_rate": np.mean([r["recorded_feasible_success"] for r in results]).item(),
    ...
    for metric in ("time_s", "path_m", "energy_proxy_j", "handovers", "outage_s", "dropped_bits",
                   "delay_proxy_mean_s", "interference_mean_w", "sinr_mean_db", "sinr_p05_db"):
...
def train(radio, scenarios, validation, reward, terminate_success, seed, output,
    ...
    env = UAVBatch(radio, config.environments, scenarios, reward, terminate_success, env_config)
    obs, mask = env.observe()
    ...
    for update in range(1, updates + 1):
        ...
        for _ in range(config.rollout):
            ...
            obs, mask, raw_reward, done, completed = env.step(torch.tanh(latent).numpy(), choice.numpy())
