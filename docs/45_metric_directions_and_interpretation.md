...
| --- | --- | --- | --- |
| Joint success | Higher is better | Primary: reaching and stopping at the goal within time and sampled connectivity limits. | Mission and constraints: printed pp. 10-11, Eqs. (11)-(12); comparable source success rate NR. |
| Flight time | Lower is better on common successes | An early failed flight is not a faster completed mission. | Source does not terminate on arrival, printed p. 12, Sec. 5.2. |
| SNR and SINR (dB), reported separately | Higher is better | SNR compares the desired signal with noise; SINR also includes interference. Higher SINR means a cleaner radio link. V2's lower SINR indicates worse signal quality on the matched successful flights. | Printed p. 10, Eq. (8); p. 18, Fig. 6; p. 20, Fig. 8. |
| Outage and failures | Lower is better | Zero outage among successes follows from the success definition; all failures still count. | Original outage bars: printed pp. 18, 20, Figs. 6, 8. |
| Downlink interference power (µW) | Lower is better | Unwanted cochannel power received at the UAV from other base stations. The small V2 reduction is inconclusive. The source supplies no corresponding downlink result. | No matched result at printed p. 10, Eq. (9); pp. 18, 20, Figs. 6, 8. |
| Source uplink interference (µW) | Lower is better within the source definition | Approximate CDF medians of the interference metric labeled uplink by the thesis. Our study has no matched uplink evaluation, so its cells are marked not evaluated. | Printed p. 10, Eq. (9); pp. 18, 20, Figs. 6, 8. |
| Consumed energy (kJ) | Lower is better | Estimated energy spent completing a successful mission under our engineering model. V2 consumes more on the matched flights. No compatible source consumed energy result is available. | Different energy model: printed p. 10, Eq. (10); p. 15, Table 3. |
| Source remaining energy display (printed kW) | Higher is better in the intended remaining energy interpretation; unit unresolved | The source labels an energy display with a power unit. Its bar readings remain in a separate row and cannot support a physical energy comparison. | Printed p. 10, Eq. (10); p. 15, Table 3; pp. 18, 20, Figs. 6, 8. |
| Handovers | Lower is better while maintaining service | Fewer switches can reduce control overhead, but excessive reluctance to switch can worsen service. | Source counts are shown on an unexplained scaled axis, printed pp. 18, 20, Figs. 6, 8. |
| Delay proxy | Lower is better | Backlog divided by capacity, not a tracked packet latency distribution. | Printed p. 9, Eq. (4); numerical source delay results NR. |
