...
SOURCES = ["src/uav_joint_optimization/connectivity_env.py", "src/uav_joint_optimization/connectivity_ppo.py",
           "src/uav_joint_optimization/connectivity_planner.py",
           "scripts/run_connectivity_experiment.py", "configs/connectivity_scenarios_v2.json",
           "scripts/analyze_connectivity_experiment.py",
...
def splits():
    if not SPLITS.exists():
        spec = {"training": make_scenarios(4096, 52010, 200, 1800), "validation": make_scenarios(64, 52001),
...
def main():
    ...
    parser.add_argument("--controllers", nargs="+", default=["straight_rss", "straight_radio", "joint_mpc", "joint_lookahead"],
                        choices=["straight_rss", "straight_radio", "joint_mpc", "joint_lookahead"])
    ...
    if args.phase == "freeze":
        ...
        FREEZE.write_text(json.dumps({"frozen_utc": datetime.now(timezone.utc).isoformat(),
            "seeds": args.seeds, "arms": args.arms, "controllers": args.controllers, "environment": asdict(cfg), "ppo": asdict(ppo),
    ...
    if args.phase == "confirmatory":
        ...
        assert args.controllers == frozen["controllers"]
