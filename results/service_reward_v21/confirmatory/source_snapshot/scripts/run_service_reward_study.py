...
SOURCES=['src/uav_joint_optimization/service_reward_env.py',
         ...
         'scripts/evaluate_service_reward_controller.py',
         'tests/test_service_reward.py','docs/46_service_reward_development_protocol.md']


def configure_label(label):
    """Keep complete local reproductions separate from delivered artifacts."""
    global BASE, FREEZE, SPLITS
    if label is not None:
        if not re.fullmatch(r'[A-Za-z0-9_]+',label): raise ValueError('Use letters, digits and underscores for the label.')
        BASE=ROOT/'results'/f'service_reward_{label}'
        FREEZE=ROOT/'configs'/f'frozen_service_reward_{label}.json'
        SPLITS=ROOT/'configs'/f'service_reward_scenarios_{label}.json'
    return BASE,FREEZE,SPLITS
...
def main():
    ...
    parser.add_argument('--label',help='Fresh reproduction label; omit for the delivered V2.1 study.')
    args=parser.parse_args(); configure_label(args.label); verify_history(); torch.set_num_threads(2)
